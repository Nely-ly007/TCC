...
public static class HiveTilePaletteBuilder
{
    const string PaletteName = "HivePalette_Auto";
    ...
    {
        foreach (var g in AssetDatabase.FindAssets("HiveTilePaletteBuilder t:MonoScript"))
        {
            string p = AssetDatabase.GUIDToAssetPath(g);
            if (Path.GetFileNameWithoutExtension(p) != "HiveTilePaletteBuilder") continue;
            ...
            {
                Debug.LogError("[HiveTilePalette] Pasta nao encontrada: " + SpritesDir + ". A pasta 'Sprites' precisa ficar ao lado de 'Editor'.");
                return false;
            }
            Debug.Log("[HiveTilePalette] Raiz detectada: " + Root);
            return true;
        }
        Debug.LogError("[HiveTilePalette] Nao achei o script HiveTilePaletteBuilder.cs no projeto.");
        return false;
    }

    [MenuItem("Tools/Hive Tile Palette/1. Construir Tudo")]
    public static void BuildAll()
    {
        if (!FindRoot()) return;
        int sprites = ImportSpritesInternal();
        if (sprites == 0) { Debug.LogError("[HiveTilePalette] Nenhum PNG encontrado em " + SpritesDir); return; }
        int tiles = CreateTiles();
        int placed = CreatePalette();
        Debug.Log("[HiveTilePalette] PNGs: " + sprites + " | Tiles criados: " + tiles + " | Tiles na paleta: " + placed
                  + ". Abra Window > 2D > Tile Palette e escolha '" + PaletteName + "'.");
    }

    [MenuItem("Tools/Hive Tile Palette/2. So reimportar sprites")]
    ...
    static int CreateTiles()
    {
        ...
        foreach (var path in GetPngs())
        {
            ...
            if (sprite == null) { Debug.LogWarning("[HiveTilePalette] Sem sprite em: " + path); continue; }
            ...
            tile.colliderType = ColliderFor(cat, Path.GetFileNameWithoutExtension(path));
    ...
    static Tile.ColliderType ColliderFor(string cat, string file)
    {
        if (cat.StartsWith("01_") && (file.Contains("_fina_") || file.Contains("rampa"))) return Tile.ColliderType.Sprite;
    ...
    [MenuItem("Tools/Hive Tile Palette/3. Preencher paleta selecionada")]
    public static void FillSelectedPalette()
    {
        ...
        {
            Debug.LogError("[HiveTilePalette] Selecione o prefab da paleta no Project.");
            return;
        }
        int n = FillPalette(p);
        Debug.Log("[HiveTilePalette] Tiles colocados: " + n);
    }
    ...
    static int FillPalette(string prefabPath)
    {
        ...
        if (map == null)
        {
            Debug.LogError("[HiveTilePalette] Nenhum Tilemap dentro de " + prefabPath);
